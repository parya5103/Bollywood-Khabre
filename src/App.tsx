...
export default function App() {
  ...
  const RailItem = ({ icon: Icon, label, active, onClick }: { 
    ...
    <button
      ...
      className={cn(
        "group relative flex items-center justify-center w-14 h-14 rounded-2xl transition-all duration-500 focus-visible:ring-2 focus-visible:ring-accent outline-none",
        active 
    ...
    <div className="min-h-screen bg-dark-surface text-slate-100 flex font-sans selection:bg-accent/30 tracking-tight">
      ...
      <nav className="fixed left-0 top-0 bottom-0 w-20 flex flex-col items-center py-8 glass border-r border-white/5 z-[60]">
        ...
        <div className="flex flex-col gap-4">
          <button onClick={() => setDarkMode(!darkMode)} aria-label={darkMode ? 'Switch to light mode' : 'Switch to dark mode'} className="w-10 h-10 flex items-center justify-center rounded-xl text-slate-500 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-accent outline-none">
      ...
      <main className="flex-1 ml-20 flex flex-col min-h-screen relative z-10">
        ...
        <header className="sticky top-0 z-50 px-8 py-6 flex items-center justify-between border-b border-white/5 bg-dark-surface/80 backdrop-blur-3xl">
          ...
          <div className="flex items-center gap-6">
             ...
             <button aria-label="Notifications" className="relative w-10 h-10 flex items-center justify-center text-slate-500 hover:text-white transition-all focus-visible:ring-2 focus-visible:ring-accent outline-none rounded-xl">
               <Bell size={20} />
